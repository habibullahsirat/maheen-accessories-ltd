...
export async function PATCH(
  ...
  try {
    const updated = await FeatureSection.findByIdAndUpdate(id, data, {
...
export async function DELETE(
  ...
  try {
    await connectToDB();
    const deleted = await FeatureSection.findByIdAndDelete(id);
    if (!deleted) {
      return NextResponse.json(
        { message: "Feature data not found" },
        { status: 404 },
    ...
    console.error(error);
    return NextResponse.json(
      { message: "Failed to delete feature data" },
      { status: 500 },
    );
  }
}
