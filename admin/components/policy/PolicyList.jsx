...
export default function PolicyList({ policy, onEdit, onDelete, isDeleting }) {
  const [imageErrors, setImageErrors] = useState({});

  const handleImageError = (policyId) => {
    setImageErrors((prev) => ({ ...prev, [policyId]: true }));
  };
      ...
      {policy.map((policy) => (
        ...
        >
          ...
          <div className="relative h-48 w-full bg-gray-100">
            {!imageErrors[policy._id] ? (
              <Image
                src={policy.image}
                alt={policy.title}
                fill
                className="object-cover"
                onError={() => handleImageError(policy._id)}
            ...
            {policy.image?.includes("cloudinary") && (
          ...
          <div className="p-4">
            ...
            {/* CTA Display */}
            {policy.cta && (policy.cta.text || policy.cta.href) && (
            ...
            <div className="flex items-center justify-between text-xs text-gray-400 mb-4">
              <span>ID: {policy._id.slice(-6)}</span>
              <span>
